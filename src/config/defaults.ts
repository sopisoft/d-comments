...
export const defaultConfigs = defineConfigs({
   ...
   * 共通の設定
   */
  login: { ui_type: 'switch', value: false },
  enable_auto_play: { ui_type: 'switch', value: true },
  auto_search: { ui_type: 'switch', value: false },
  enable_auto_scroll: { ui_type: 'switch', value: true },

  /**
   * サイドバーの設定
   */

  show_comments_in_list: { ui_type: 'switch', value: true },
  comment_area_width_px: {
    ui_options: { min: 100, max: 2000, step: 1 } satisfies UiOptions,
    ui_type: 'number',
    value: 600,
  },
  comment_area_font_size_px: { ui_options: { min: 2, max: 40 } satisfies UiOptions, ui_type: 'number', value: 16 },
  comment_area_opacity_percentage: {
    ui_options: { min: 0, max: 100, step: 5, unit: '%' } satisfies UiOptions,
    ui_type: 'slider',
    value: 95,
  },

  nicoarea_scale: {
    ui_options: { min: 0, max: 100, step: 10, unit: '%' } satisfies UiOptions,
    ui_type: 'slider',
    value: 100,
  },

  show_nicoru_count: { ui_type: 'switch', value: true },
  show_comments_in_niconico_style: { ui_type: 'switch', value: true },
  visible_comments: {
  ...
  channels_only: { ui_type: 'switch', value: true },
  enable_smooth_scrolling: { ui_type: 'switch', value: false },
  comment_timing_offset: {
    ui_options: { min: -100000, max: 100000, step: 10 } satisfies UiOptions,
    ui_type: 'number',
    value: 0,
  },
  comment_renderer_fps: {
    ui_options: { min: 15, max: 120, step: 5, unit: 'fps' } satisfies UiOptions,
    ui_type: 'slider',
    value: 60,
  },
  ...
  addon_option_play_in_same_tab: { ui_type: 'switch', value: true },
  ng_user_ids: { ui_type: 'checkbox_group', value: [] as Array<{ value: string; enabled: boolean }> },
  ng_words: { ui_type: 'checkbox_group', value: [] as Array<{ value: string; enabled: boolean; isRegex?: boolean }> },
});
